...
export function ApplicationReviewPage() {
  ...
  useEffect(() => {
    const fetchApplication = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${API_BASE}/api/project/application/${applicationId}`);
            if (res.data.success) {
          ...
          <div className="glass-panel rounded-xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-white font-sans">{(application as any).experience || "Not specified"}</p>
              </div>
              <div>
                <h3 className="text-sm text-gray-400 font-sans mb-2">Skills</h3>
                <div className="flex flex-wrap gap-2">
                    {(application as any).skills?.map((skill: string) => (
                        <span key={skill} className="px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 text-xs border border-cyan-500/20">
                            {skill}
                        </span>
                    ))}
                </div>
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-white font-sans">{(application as any).bio || "No bio provided"}</p>
              </div>
            </div>
