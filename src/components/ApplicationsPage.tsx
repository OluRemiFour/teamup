    ...
    const fetchApplications = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/api/collaborators/applications`,
        );
        if (res.data.success) {
            ...
            >
              <div className="flex items-start gap-4">
                <Avatar className="w-12 h-12">
                  ...
                  />
                  <AvatarFallback>
                    {application.project.owner.name[0]}
                  </AvatarFallback>
                </Avatar>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-400 font-sans">
                        {application.project.company} • Posted by{" "}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono border ${getStatusColor(application.status)}`}
                      >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/10">
                    ...
                    <div className="flex gap-2">
                      {application.status === "accepted" ? (
                        <Link
                          to={`/dashboard/messages?project=${application.project.id}`}
                        >
                          <Button
                            size="sm"
                            className="bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-400 hover:to-purple-400 font-sans"
                          >
                        ...
                        <Link
                          to={`/dashboard/messages?recipient=${application.project.owner.name}`}
                        >
                          <Button
                            size="sm"
                            variant="outline"
