...
export function MyProjectsPage() {
  ...
  const handleArchive = async (projectId: string) => {
    try {
      ...
      const res = await axios.put(
        `${API_BASE}/api/project/archive-projects/${projectId}`,
      );
      if (res.status === 200) {
        toast({
          title: "Archived",
  ...
  const handleUnarchive = async (projectId: string) => {
    try {
      setActionLoadingId(projectId);
      const res = await axios.put(
        `${API_BASE}/api/project/unarchive-projects/${projectId}`,
      );
      if (res.status === 200) {
        toast({
          title: "Unarchived",
  ...
  const handleCompleteSelection = async (projectId: string) => {
    try {
      ...
      if (res.data.success) {
        toast({
          title: "Team Selection Finished! 🎯",
          description: "Your project is now ongoing.",
        });
        const updated = projects.map((p) =>
          p.id === projectId ? { ...p, lifecycleStage: "ongoing" as const } : p,
        );
        setProjects(updated);
        setCachedData(cacheKey, updated);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {(["all", "active", "completed", "draft", "archived"] as const).map(
          (status) => (
            <button
              key={status}
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <Link
                      ...
                      className="hover:underline decoration-cyan-400 underline-offset-4"
                    >
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-mono capitalize ${
                        project.lifecycleStage === "ongoing"
                          ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20"
                          : project.lifecycleStage === "team-search"
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-white/10">
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  {project.lifecycleStage !== "team-search" &&
                    project.status === "active" && (
                      <Link
                        to={`/dashboard/project/${project.id}?tab=management`}
                      >
                        <Button
                          size="sm"
                          className="bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20 font-sans"
                        >
                  ...
                  <Link to={`/dashboard/projects/${project.id}/applicants`}>
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-white/20 text-gray-300 hover:text-white hover:border-white/40 font-sans"
                    >
                      View Applicants
                    </Button>
                  </Link>
        ...
        <AlertDialogContent className="glass-panel border-white/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-400 font-sans">
              ...
              be undone. All applicants and project data will be permanently
              removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
