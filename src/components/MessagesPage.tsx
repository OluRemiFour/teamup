...
export function MessagesPage() {
  ...
  useEffect(() => {
    ...
    if (projectId) {
      const conv = conversations.find(c => c.project && c.project.includes(projectId)); // Simplification, backend might return title
      ...
      const conv = conversations.find(c => 
        c.participant.name.toLowerCase() === recipientName.toLowerCase()
      );
      if (conv) {
        setSelectedConversation(conv);
      }
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
        try {
            const res = await axios.get(`${API_BASE}/api/messages/${selectedConversation.id}`);
            if (res.data.success) {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-gray-500 font-mono">
                        {new Date(conversation.timestamp).toLocaleDateString() === new Date().toLocaleDateString() 
                ...
                <DropdownMenuContent align="end" className="bg-[#1a1f2e] border-white/10">
                  <DropdownMenuItem 
                    ...
                    onClick={async () => {
                        if (!selectedConversation) return;
                        try {
                            await axios.patch(`${API_BASE}/api/messages/${selectedConversation.id}/read`);
                            setConversations(prev => prev.map(c => 
                                c.id === selectedConversation.id ? { ...c, unread: 0 } : c
                            ));
                            toast({ title: "Marked as Read", description: "Conversation marked as read." });
                        } catch (error) {
                  ...
                  <DropdownMenuItem 
                    ...
                    onClick={async () => {
                        if (!selectedConversation) return;
                        try {
                            await axios.delete(`${API_BASE}/api/messages/${selectedConversation.id}`);
                            setConversations(prev => prev.filter(c => c.id !== selectedConversation.id));
                            setSelectedConversation(null);
                            toast({ title: "Conversation Deleted", description: "This conversation has been removed." });
                        } catch (error) {
